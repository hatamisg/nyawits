...
final class FieldMappingStore: ObservableObject {
    ...
    init(fileManager: FileManager = .default, storageRoot: URL? = nil) {
        ...
        rootURL = storageRoot ?? applicationSupport.appendingPathComponent("NyawitsMapping", isDirectory: true)
        photosURL = rootURL.appendingPathComponent("PlantPhotos", isDirectory: true)
        dataURL = rootURL.appendingPathComponent("mapped-fields.json")
        selectionURL = rootURL.appendingPathComponent("active-field.json")
    ...
    func deleteField(id: UUID) -> Result<FieldDeleteOutcome, FieldStoreError> {
        ...
        if activeFieldID == id {
            let fallback = FieldListPresentation
                .sortFields(candidates.filter { $0.isDemo != true })
                .first?.id
            do {
    ...
    private func load() {
        ...
        do {
            ...
            fields = try decoder.decode([MappedField].self, from: data)
    ...
    private func loadSelectionSidecar() {
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(ActiveFieldSidecar.self, from: data)
    ...
    private func normalizeActiveSelection(stored: UUID?, sidecarExisted: Bool) {
        ...
        let fallback = FieldListPresentation.sortFields(realFields).first?.id
    ...
    private func writeSelectionSidecar(fieldID: UUID?, isDemo: Bool = false) throws {
        let data = try JSONEncoder().encode(ActiveFieldSidecar(fieldID: fieldID, isDemo: isDemo ? true : nil))
    ...
    private func safeDeletePhoto(named filename: String) -> Bool {
        guard filename == (filename as NSString).lastPathComponent,
              !filename.isEmpty, filename != ".", filename != ".." else { return false }
        let url = photosURL.appendingPathComponent(filename)
